...
const FLAG_CLASS: Record<RuleLevel, string> = {
  override:
    "mt-3.5 border-coral/40 bg-coral/12 px-2.5 py-1 text-coral",
  warn: "mt-2 mr-1.5 border-amber/35 bg-amber/10 px-2 py-1 text-amber",
  info: "mt-2 mr-1.5 border-teal/30 bg-teal/8 px-2 py-1 text-teal",
};
...
function BannerFrame({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <section
      className={cn(
        "relative mb-5 overflow-hidden rounded-xs border border-line bg-gradient-to-br from-panel to-panel-2 p-6",
        className,
      )}
    >
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -top-[40%] -right-[10%] size-64 rounded-full border border-line opacity-50"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -top-[15%] right-[5%] size-36 rounded-full border border-line opacity-50"
      />
      {children}
    </section>
  );
...
export function RecommendationBanner({
  ...
  return (
    <>
      <BannerFrame>
        <div className="mb-2.5 font-mono text-xs uppercase tracking-widest text-ink-faint">
          Khuyến nghị của hệ thống
        </div>
        <h2
          className={cn(
            "relative z-1 m-0 mb-2.5 font-serif text-3xl font-medium",
        ...
        <p className="relative z-1 m-0 max-w-2xl text-sm text-ink-dim">{verdict.text}</p>
        <div>
          {flags.map((f) => (
            <span
              key={`${f.id}-${f.level}`}
              className={cn(
                "relative z-1 inline-block rounded-xs border font-mono text-xs",
                FLAG_CLASS[f.level],
              )}
            >
              [{f.id}] {f.msg}
      ...
      {ecoPreferred && ecoOverride && (
        <BannerFrame className="border-teal/45">
          ...
          <p className="relative z-1 m-0 max-w-2xl text-sm text-teal">
